...
export default function AdminStorePendingOrdersPage() {
  ...
  const handlePendingAction = async (orderId: string, action: 'approve' | 'reject') => {
    setPendingSaving(orderId);
    setError(null);

    const reason = action === 'reject' ? window.prompt('Red sebebi (opsiyonel):') ?? '' : '';
              ...
              <div key={order.id} className="rounded-xl border border-white/10 bg-[#0b0d12]/60 p-4">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  <button
                    ...
                    className="rounded-full bg-emerald-500/80 px-3 py-1 text-[11px] font-semibold text-white transition hover:bg-emerald-400 disabled:cursor-not-allowed disabled:opacity-60"
                  >
                    Onayla
                  </button>
                  <button
                    type="button"
                    onClick={() => handlePendingAction(order.id, 'reject')}
                    disabled={pendingSaving === order.id}
                    className="rounded-full bg-rose-500/80 px-3 py-1 text-[11px] font-semibold text-white transition hover:bg-rose-400 disabled:cursor-not-allowed disabled:opacity-60"
                  >
                    Reddet
                  </button>
                </div>
